...
const OthersProjectCard = ({
  ...
  return (
    ...
    >
      <div className="h-40 sm:h-80 bg-gray-200 hover:bg-[#1c295a] rounded-lg flex sm:flex-col justify-between items-center px-8 py-4 sm:px-4 sm:py-8 sm:text-center sm:gap-5">
        <div className="w-1/2 ">
        <img
          src={imagePath}
          alt="personal brands"
      ...
      {hover && (
        <div className="absolute top-0 right-0 w-full h-full bg-gray-800 text-white bg-opacity-90 flex justify-center items-center rounded-lg" style={{border:'solid 8px #1D2942'}}>
          <a
            href={url}
            className="px-8 py-2 rounded-lg bg-[#fff] text-black font-bold"
            onTouchStart={(e) => {
              e.stopPropagation();
              setHover(true);
            }}
